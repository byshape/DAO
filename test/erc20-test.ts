...
describe("Token20", () => {
    ...
    const setupTest = deployments.createFixture(
        async ({deployments, getNamedAccounts, ethers}, options) => {
            ...
            const TokenContract = await ethers.getContract("VoteToken", deployer);
    ...
    it("Does not mint tokens to the zero address",async () => {
        ...
        await expect(erc20.mint(zeroAddress, amountToMint)).to.be.revertedWith("Mint to the zero address");
        expect(await erc20.balanceOf(zeroAddress)).to.be.equal(0);
    });
    ...
    it("Mints tokens by the owner",async () => {
        let amountToMint: bigint = BigInt(10);
        expect(await erc20.balanceOf(user.address)).to.be.equal(0);
        await expect(erc20.mint(user.address, amountToMint)).to.emit(erc20, "Transfer").withArgs(zeroAddress, user.address, amountToMint);
